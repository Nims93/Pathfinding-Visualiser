...
var tutorialBtn = document.querySelector('#help'); //class represents each node/vertex in grid
...
var Node = /*#__PURE__*/function () {
  function Node(id) {
    ...
    this.id = id;
    this.index = id.split('-').map(function (n) {
      return Number(n);
    });
...
function createGrid(gridSize, visualiserWrapper) {
  var wrapperWidth = visualiserWrapper.clientWidth;
  var wrapperHeight = visualiserWrapper.clientHeight;
  var gridWidth = gridSize;
  var gridHeight = Math.floor(gridSize * 0.45); // console.log(wrapperWidth);
  ...
  if (!e.target.classList.contains('visualiser') && grid.canMutate) {
    ...
    if (e.target != prevEle) {
      if (e.buttons === 1 && prevEle.classList.contains('start-node')) {
        ...
        prevEle = DOMEle;
        if (grid.previousAlgo) findPathOnStartOrEndNodeMouseDrag(grid.previousAlgo);
...
function findPathOnStartOrEndNodeMouseDrag(value) {
  if (grid.canMutate) {
    var prevAnimationSpeed = grid.animationSpeed;
    grid.animationSpeed = 0;
...
var closeTutorialBtnArray = document.querySelectorAll('.close-tutorial');
tutorialBtn.addEventListener('click', function () {
  var tutorialTransparencyLayer = document.querySelector('.tutorial-transparency-layer');
  tutorialTransparencyLayer.classList.add('visible');

  var _iterator = _createForOfIteratorHelper(closeTutorialBtnArray),
      _step;
...
var tutorialNextAndPrevBtns = document.querySelectorAll('.tutorial-switcher-btn');

var _iterator2 = _createForOfIteratorHelper(tutorialNextAndPrevBtns),
    _step2;
...
function handleNextAndPreTutorialClick(e) {
  var pageChangeBtn = e.currentTarget;
  var tutorialWrapper = document.querySelector('.tutorial-wrapper');

  for (var i = 0; i < tutorialWrapper.children.length; i++) {
  ...
  if (pageChangeBtn.classList[0] === 'next-tutorial-slide') {
    currVisChildIdx + 1 < tutorialWrapper.children.length ? tutorialWrapper.children[currVisChildIdx + 1].classList.add('visible') : tutorialWrapper.children[0].classList.add('visible');
  } else {
    currVisChildIdx > 0 ? tutorialWrapper.children[currVisChildIdx - 1].classList.add('visible') : tutorialWrapper.children[tutorialWrapper.children.length - 1].classList.add('visible');
  }
} //-----------------------------------------------------------------------------
//
//=============================================================================
  ...
  function divide(yStart, yEnd, xStart, xEnd, orient) {
    ...
    if (orient === 'horizontal') {
      ...
      if (previousHoles[yDivideCoord][xStart - 1]) {
        ...
        holeCoord = Math.random() > 0.5 ? xStart : xEnd; // holeCoord = randInt(xEnd, xStart);
      }
      ...
      if (previousHoles[yStart - 1][xDivideCoord]) {
        ...
        _holeCoord = Math.random() > 0.5 ? yStart : yEnd; // holeCoord = randInt(yEnd, yStart);
      }
...
function aStar() {
  ...
  while (!priorityQueue.isEmpty()) {
    ...
    var _iterator4 = _createForOfIteratorHelper(neighbors),
        _step4;
    ...
    try {
      for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
        ...
        var newGlobalCost = newNeighborDist + neighborHorizDist;
...
function breadthFirstSearch() {
  ...
  while (queue.length) {
    ...
    var _iterator5 = _createForOfIteratorHelper(neighbors),
        _step5;
...
function biDirectionalBreadthFirstSearch() {
  ...
  while (endNodeQueue.length && startNodeQueue.length) {
    ...
    if (intersection) {
      ...
      var pathNodesToDisplay = shortestPathStart.concat(shortestPathEnd.reverse());
    ...
    var _iterator6 = _createForOfIteratorHelper(startNeighbors),
        _step6;
    ...
    var _iterator7 = _createForOfIteratorHelper(endNeighbors),
        _step7;

    try {
      for (_iterator7.s(); !(_step7 = _iterator7.n()).done;) {
        var _neighbor = _step7.value;
        if (endVisited[_neighbor.y][_neighbor.x]) continue;
        endVisited[_neighbor.y][_neighbor.x] = true;
        visitedNodesToDisplay.push(_neighbor);
...
function depthFirstSearch() {
  ...
  while (stack.length) {
    ...
    var _iterator8 = _createForOfIteratorHelper(neighbors),
        _step8;
